class Solution:
    def kClosest(self, points: List[List[int]], k: int) -> List[List[int]]:
        ...
        for p in points:
            ...
            distance = -(p[0]**2 + p[1]**2)
            heapq.heappush(max_heap, [distance, p[0], p[1]]) # O(n*log k ) -> log K because 
            # the heap can only accomodate k elements (or max K+1 befire we run the next if block).

            # In the loop, limit the heap to max k elements
            # and keep on poping elements if the len of the heap is greater than K
            if len(max_heap) > k:
                heapq.heappop(max_heap) # O((n-k) * log k)

        # Iterate the heap and keep on popping min values and adding to the list
        res = [] # O(k)
        while len(max_heap)>0:
            distance, x, y = heapq.heappop(max_heap) # O(k * log k)
            res.append([x, y])

        # Time Complexity: n * log k + (n-k)log k + k*log k = O(n log k)
        # Space: O(k)
